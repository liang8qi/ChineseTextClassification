...
import numpy as np
from text_drnn_model import TDRNNConfig, TDRNN
import os
import time
...
def batch_iter(x, y, batch_size=128):
    ...
    for i in range(num_batch):
        ...
        x_shuffle_padded = kr.preprocessing.sequence.pad_sequences(x[start_id:end_id], maxlen=max_len,
                                                                   padding="pre", truncating="post")

        yield x_shuffle_padded, y[start_id:end_id]


def feed_data(x, y):
    sequence_lengths = data.get_sequence_length(x)
    feed_dict = {
        model.input_x: x,
        model.input_y: y,
