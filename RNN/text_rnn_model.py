...
class TextRNN(object):
    ...
    def __init__(self, config):
        ...
        with tf.name_scope("RNN"):
            ...
            lstm_cells = [self.dropout() for _ in range(self.config.num_layers)]

            rnn_cell = tf.contrib.rnn.MultiRNNCell(lstm_cells)
            # print("rnn_cell: {}".format(tf.shape(rnn_cell)))
            # outputs shape: [batch_size, text_len, hidden_dim)
            outputs, states = tf.nn.dynamic_rnn(cell=rnn_cell, inputs=self.embedding_inputs, dtype=tf.float32,
                                                sequence_length=self.input_text_len)
            print(outputs)
