...
from text_rnn_model import TRNNConfig, TextRNN
import numpy as np
import time
...
def batch_iter(x, y, batch_size=128):
    ...
    for i in range(num_batch):
        ...
        x_padded = kr.preprocessing.sequence.pad_sequences(x_shuffle[start_id:end_id], maxlen=max_len,
    ...
    for epoch in range(config.num_epochs):
        ...
        for x_batch, y_batch in batch_train:
            ...
            if total_batch % config.print_per_batch == 0:
                ...
                # print(session.run(model.embedding_inputs))
                # embedding_inputs = session.run([model.embedding_inputs], feed_dict=feed_dict)
...
if __name__ == "__main__":
    ...
    model_src = "data/model/2_layer_lstm_model"
